...
parser.add_argument('--specfile', required=True, help='File containing the specification for the generator')
parser.add_argument('--output', required=True, help='Output directory for generator results')
parser.add_argument('--platform', required=True, help='PDK/process kit for cadre flow (.e.g tsmc16lp)')
parser.add_argument('--mode', default='verilog', choices=['verilog', 'macro', 'full'], help='Switched-Cap DC-DC Gen operation mode. Default mode: \'verilog\'.')
...
filedata = re.sub(r'parameter DCDC_NUM_STAGE = \d+;', r'parameter DCDC_NUM_STAGE = ' + str(dcdc_num_stage) + ';', filedata)
filedata = re.sub(r'parameter DCDC_CONFIG = \d+;', r'parameter DCDC_CONFIG = ' + str(dcdc_config) + ';', filedata)
...
if args.mode == 'verilog':
   ...
   shutil.copy(verilogDir + '/DCDC_HUNIT_CONV2to1.v', flowDir + '/src/' + '/DCDC_HUNIT_CONV2to1.v')

   # SDC file Modification
   if args.platform == 'gf12lp':
      period = 1.0E+12/(Frequency*1000)
   else:
      period = 1.0E+9/(Frequency*1000)
   clk_tran = period/20
   ...
   filedata = re.sub(r'set num_stage .*'   , r'set num_stage '    + str(dcdc_num_stage)    , filedata)
   filedata = re.sub(r'set config .*'      , r'set config '       + str(dcdc_config)       , filedata)
   filedata = re.sub(r'set cap_size .*'    , r'set cap_size '     + str(dcdc_cap_size)     , filedata)
   ...
   filedata = re.sub(r'set pg_via_hv .*'   , r'set pg_via_hv "'   + str(pg_via_hv) + '"'   , filedata)
   filedata = re.sub(r'set pg_unit_cap .*' , r'set pg_unit_cap "' + str(pg_unit_cap) + '"' , filedata)
...
if args.mode != 'verilog':
   ...
   cfg.dcdc_gen_dg_flow_cfg(args.platform, platformConfig['aux_lib'], designName, flowDir)
...
if args.mode != 'verilog':
   ...
   p.wait()
   p = sp.Popen(['cp', flowDir+'/export/'+designName+'_typ.lib', \
                 args.output+'/'+designName+'.lib'])
   p.wait()
   p = sp.Popen(['cp', flowDir+'/export/'+designName+'_typ.db', \
