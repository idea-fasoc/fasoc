...
//	`include "functions.v"
	...
	localparam FILTER_GAIN_FRAC_WIDTH = func_MAX(KP_FRAC_WIDTH,KI_FRAC_WIDTH); // 10
	...
	localparam FILTER_ACCUM_FRAC_WIDTH = FILTER_GAIN_FRAC_WIDTH + TDC_ROUND_WIDTH; //km-test: 10 + 9 = 19
...
//Internal Signals
	...
	logic							freq_lock_neg_vio;
	logic							freq_lock_pos_vio;
//****************************************************************************************\\
//                              State machine                                             \\
//****************************************************************************************\\
  // FSMs
	...
	always @(posedge CLKREF_RETIMED or posedge RST) begin
		if (RST) 
			begin 
				...
				filter_accum_d <= $unsigned(DCO_FCW_MAX_Q)<<<(FILTER_ACCUM_FRAC_WIDTH - FILTER_OUT_FRAC + SD_WIDTH); // test v2: fractional bit : 19 - 6 - 3 ?? 
			end
		else
			begin
...
// Generate the DCO control words
	...
	always @* begin
		...
		filter_out_int = filter_out >>> (FILTER_ACCUM_FRAC_WIDTH - FILTER_OUT_FRAC); //km-test dsm : fractional part left: 10 - 6 = 4
		...
		{dco_fcw_filter,fine_dither_filter} = dco_control_word[DCO_FCW_BIN_WIDTH+1+SD_WIDTH-1:0]; // fractional part that got converted to integer part: 4 - 3 = 1 
	...
	assign freq_lock_neg_vio = (phase_ramp_error > -FREQ_LOCK_THRESHOLD)? 0 : 1; 
	assign freq_lock_pos_vio = (phase_ramp_error < FREQ_LOCK_THRESHOLD)? 0 : 1;
