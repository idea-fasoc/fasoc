...
def gen_tb(PDK,tb_dir,format_dir,ncell,ndrv,nfc,nstg_start,nstg_end,nstg_step,vdd,temp,nper):
	#=====================================================
...
#	sys.setrecursionlimit(N_recur+1)  #expand the recursion limit if exceeded
	...
	for i in range(1,len(vm1.comblist[0])):
		...
		netmap1.get_net('nd',None,vm1.comblist[2][i],vm1.comblist[2][i],1) #ndrv
		netmap1.get_net('nm',None,vm1.comblist[0][i],vm1.comblist[0][i],1) #ncell
		netmap1.get_net('nt','sc',vm1.comblist[1][i],vm1.comblist[1][i],1) #nstage
		...
		#vm2=HSPICE_mds.varmap()   ###modify here!!
		##vm2.get_var('fc',0,nfc,nfc//4)           #vm2.comblist[0]=nfc
		#vm2.get_var('fc',0,N_ctrl_fc,N_ctrl_fc)           #vm2.comblist[0]=nfc
		##vm2.get_var('cc',0,ncell,ncell//2)       #vm2.comblist[1]=ncc
		#vm2.get_var('cc',0,1,1)       #vm2.comblist[1]=ncc
		#vm2.add_val('cc',N_ctrl_cc-1,N_ctrl_cc,1)       #vm2.comblist[1]=ncc
		#vm2.get_var('vdd',vdd[0],vdd[0],1)     #vm2.comblist[2]=vdd   #???? why like this? because of get_var 
		#if len(vdd)>1:
		#	for i in range(1,len(vdd)):
		#		vm2.add_val('vdd',vdd[i],vdd[i],1)
		#vm2.get_var('temp',temp[0],temp[0],1)  #vm2.comblist[3]=temp
		#if len(temp)>1:
		#	for i in range(1,len(temp)):
		#		vm2.add_val('temp',temp[i],temp[i],1)
		#vm2.cal_nbigcy()
		...
		with open(tb_dir+"tb_%dring%d_osc%d_FC%d.sp"%(vm1.comblist[2][i],vm1.comblist[0][i],vm1.comblist[1][i],nfc),"w") as w_file:
			for line in lines:
				if line[0:4]=='@L@W':
...
def gen_tb_pex(PDK,tb_dir,format_dir,ncell,ndrv,nfc,nstg_start,nstg_end,nstg_step,vdd,temp,nper,design_name,sav):
	#=====================================================
	# model constants for trans simulation time calculation 
	#=====================================================
	CF=3.805743e-13      
