...
//	`include "functions.v"
	...
	// Ports
		...
		output reg	[DCO_NUM_PHASES-1:0]	SAMPLED_RAW_PHASES;
		output 		[DCO_NUM_PHASES-1:0] 	RAW_PHASES;


	// Internal Signals
		wire 		[DCO_NUM_PHASES-1:0] 	phases_procs;
	...
	// Structural
		...
		assign DCO_OUTP = phases[0];  // test-km
	...
	// Behavioral
		...
		generate
			...
			for (genvar_ii=0;genvar_ii<DCO_NUM_PHASES;genvar_ii=genvar_ii+1) 
				begin :INV_ODD
					assign RAW_PHASES[genvar_ii] = phases[genvar_ii]; // test-km
					if (genvar_ii%2==0) begin
						assign phases_procs[genvar_ii] = phases[genvar_ii]; // test-km
					end else begin
						assign phases_procs[genvar_ii] = ~phases[genvar_ii]; // test-km
					end
				end
		...
		always @(posedge CLKREF) begin
			...
			SAMPLED_PHASES_OUT <= phases_procs;

		end

		always @(posedge CLKREF) begin

			SAMPLED_RAW_PHASES <= phases;
