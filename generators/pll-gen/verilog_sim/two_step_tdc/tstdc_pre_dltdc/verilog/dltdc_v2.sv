...
module dltdc_v2	(	tdc_out,
	...
	parameter pre_NSTG_ref_ls = 3; // test 
	parameter pre_NSTG_ref = 12; // test 

	parameter pre_fb_delay = 17e-12;
	parameter pre_NSTG_fb = 6; // edge arrives @ 17*1 = 17ps
	...
	logic	[DCO_NUM_PH-1:0]		smpl_edge_sel;
	...
	logic				npath_fb_tmp;
	logic				ppath_fb_tmp;
	logic				pre_dltdc_in_fb;
	logic				pre_dltdc_in_fb2;
	...
	generate 
		...
		begin:pre_stg_ref
			...
			//begin: driv_ref 
		 		dco_CC_se #(.CC_delay(pre_ref_delay)) DRV_ref ( .IN(pre_clk_ref[i]), .OUT(pre_clk_ref[i+1]), .EN(1'b1));
			//end 
			////Coarse Cells
			//for (j=0; j<pre_NCC_ref; j=j+1)
			//begin: cctrl_ref 
		 	//	dco_CC_se #(.CC_delay(pre_ref_delay)) CC_ref ( .IN(pre_clk_ref[i]), .OUT(pre_clk_ref[i+1]), .EN(pre_CC_ref[i+j*pre_NSTG_ref]));
			//end 
		end
	endgenerate
...
//			begin: npath_sel 
//				dco_CC_se #(.CC_delay(npath_delay)) CC ( .IN(pre_PH_fb[i][pre_NSTG_fb]), .OUT(npath_fb[0]), .EN(smpl_edge_sel[i]));
//			end 
//			for (k=0; k<ppath_NCC; k=k+1)
//			begin: ppath_sel 
//				dco_CC_se #(.CC_delay(ppath_delay)) CC ( .IN(pre_PH_fb[i][pre_NSTG_fb]), .OUT(ppath_fb[0]), .EN(smpl_edge_sel[i]));
//			end
//		end 
//	endgenerate
//
...
//
//	dco_CC_se #(.CC_delay(npath_delay)) npath_tie_hi ( .IN(1'b1), .OUT(npath_fb[0]), .EN(np_edge_off));
//	dco_CC_se #(.CC_delay(ppath_delay)) ppath_tie_lo ( .IN(1'b0), .OUT(ppath_fb[0]), .EN(np_edge_off));
	...
	generate 
		...
		begin: npath_dl
			...
			//begin: driv 
		 		dco_CC_se #(.CC_delay(npath_delay)) DRV_fb ( .IN(npath_fb[i]), .OUT(npath_fb[i+1]), .EN(1'b1));
			//end 
			////Coarse Cells
			//for (j=0; j<npath_NCC; j=j+1)
			//begin: cctrl 
		 	//	dco_CC_se #(.CC_delay(npath_delay)) CC_fb ( .IN(npath_fb[i]), .OUT(npath_fb[i+1]), .EN(npath_CC_fb[i+j*(npath_NSTG-1)]));
			//end 
		end
	endgenerate
	...
	generate 
		...
		begin: ppath_dl
			...
			//begin: driv 
		 		dco_CC_se #(.CC_delay(ppath_delay)) DRV_fb ( .IN(ppath_fb[i]), .OUT(ppath_fb[i+1]), .EN(1'b1));
			//end 
			////Coarse Cells
			//for (j=0; j<ppath_NCC; j=j+1)
			//begin: cctrl 
		 	//	dco_CC_se #(.CC_delay(ppath_delay)) CC_fb ( .IN(ppath_fb[i]), .OUT(ppath_fb[i+1]), .EN(ppath_CC_fb[i+j*(ppath_NSTG-1)]));
			//end 
		end
	endgenerate
...
// code for sim - 2: final mux
	...
	always @(posedge pre_dltdc_in_fb or negedge pre_dltdc_in_fb) begin
		PH_fb[0] <= #(fmux_delay/TIME_SCALE) ~pre_dltdc_in_fb;
	end
...
//	for (j=0; j<ppath_NCC; j=j+1)
//	begin: ppath_final
//		dco_CC_se #(.CC_delay(fmux_delay))  CC_fb ( .IN(ppath_fb[ppath_NSTG-1]), .OUT(pre_dltdc_in_fb), .EN(np_edge));
//	end
//	// npath 
//	for (j=0; j<npath_NCC; j=j+1)
//	begin: npath_final 
//		dco_CC_se #(.CC_delay(fmux_delay))  CC_fb ( .IN(npath_fb[npath_NSTG-1]), .OUT(pre_dltdc_in_fb), .EN(~np_edge));
//	end 
//	// tie-lo	
//	dco_CC_se #(.CC_delay(fmux_delay))  tie_lo_CC_fb ( .IN(1'b1), .OUT(pre_dltdc_in_fb), .EN(~np_edge_off));
//
//	dco_CC_se #(.CC_delay(fmux_delay))  buf_pre_dltdc1 ( .IN(pre_dltdc_in_fb), .OUT(pre_dltdc_in_fb2), .EN(1'b1));
//	dco_CC_se #(.CC_delay(fmux_delay))  buf_pre_dltdc2 ( .IN(pre_dltdc_in_fb), .OUT(pre_dltdc_in_fb2), .EN(1'b1));
//	dco_CC_se #(.CC_delay(fmux_delay))  buf_pre_dltdc3 ( .IN(pre_dltdc_in_fb), .OUT(pre_dltdc_in_fb2), .EN(1'b1));
//
//	dco_CC_se #(.CC_delay(fmux_delay))  buf_pre_dltdc4 ( .IN(pre_dltdc_in_fb2), .OUT(PH_fb[0]), .EN(1'b1));
//	dco_CC_se #(.CC_delay(fmux_delay))  buf_pre_dltdc5 ( .IN(pre_dltdc_in_fb2), .OUT(PH_fb[0]), .EN(1'b1));
//	dco_CC_se #(.CC_delay(fmux_delay))  buf_pre_dltdc6 ( .IN(pre_dltdc_in_fb2), .OUT(PH_fb[0]), .EN(1'b1));
	//===============================================================================
	// diff-based DLTDC 
	//===============================================================================
