...
try:
   ...
   print('Error: Bad Input Specfile. Please use a float value for \'range \' '+ \
         'under \'specifications\'.')
   sys.exit(1)



mFile1 = platformConfig['model_lib'] + '/modelfile.csv'
mFilePublic1 = genDir + '/models/' + args.platform + '.model_tempsense'
...
def read_plot_error_opt():
   #read new data
    ...
    plt.xlabel('Temperature')
    plt.ylabel('Error')
    plt.plot(search_points(inv, header)[0], search_points(inv, header)[2], 'ro', label='Other simulated points using the same design parameters')
    plt.legend(loc='upper left', frameon=True)
    plt.show()
